...
/* 测试文件 */
#define MQTT_UPLOAD_SINGLE_FILE_SIZE       (1 * 2 * 1024)
#define MQTT_UPLOAD_SINGLE_FILE_NAME       ("mqttuploadfile.txt")
uint8_t g_upload_single_file_buf[MQTT_UPLOAD_SINGLE_FILE_SIZE] = {0x01, 0x02, 0x03};
...
uint64_t demo_get_file_crc64(char *file_name) {
    uint64_t crc = 0;
    crc = upload_crc64_update(crc, g_upload_single_file_buf, sizeof(g_upload_single_file_buf));
    return crc;
}
...
void *demo_mqtt_fileupload_thread(void *args)
{
    ...
    uint64_t crc = demo_get_file_crc64(MQTT_UPLOAD_SINGLE_FILE_NAME);

    /* 配置上传文件的信息和回调，调用主动发送接口时，
    不需要配置回调，如果配置了回调参数SDK默认优先使用回调*/
    uint32_t test_userdata = 100;
    aiot_mqtt_upload_file_opt_t file_option = {
        .file_name = MQTT_UPLOAD_SINGLE_FILE_NAME,
        .file_size = MQTT_UPLOAD_SINGLE_FILE_SIZE,
