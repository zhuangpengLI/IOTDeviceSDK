/*
 * 这个例程适用于`Linux`这类支持pthread的POSIX设备, 它演示了如何上传到用户的OSS空间
...
void *demo_mqtt_fileupload_thread(void *args)
{
    ...
    /* TODO:
        如果需要上传文件到用户的OSS空间，需要配置extra_params参数，
        可以再文件管理页面查看具体文件。
        [extra_params参数配置必须使用Json格式，具体参考说明文档][https://help.aliyun.com/document_detail/354836.html]
        如果上传到物联网平台的OSS空间，需要配置extra_params为NULL，
     */
